...
bool internal_command_event_match(const hidl_vec<uint8_t>& packet) {
  ...
  if (event_code != HCI_COMMAND_COMPLETE_EVENT) {
    ...
    ALOGE("%s: HCI_EVENT_PREAMBLE_SIZE %02zX", __func__, HCI_EVENT_PREAMBLE_SIZE);
    for (int i = 0; i < packet.size(); i++)
    {
      ALOGE("%s: packet[%d] %02X", __func__, i, packet[i]);
    }
    return false;
  }
...
const bt_vendor_callbacks_t lib_callbacks = {
    sizeof(lib_callbacks), firmware_config_cb, sco_config_cb,   // size, fwcfg_cbm , scocfg_cb
    low_power_mode_cb,     sco_audiostate_cb,  buffer_alloc_cb, // lpm_cb, audio_state_cb, alloc
    buffer_free_cb,        transmit_cb,        epilog_cb,       // dealloc, xmit_cb, epilog_cb
    a2dp_offload_cb};                                           // a2dp_offload_cb
...
void VendorInterface::OnFirmwareConfigured(uint8_t result) {
  ...
  // We don't have an OnFirmwareLoaded, so...
  if (result == 2)
  {
