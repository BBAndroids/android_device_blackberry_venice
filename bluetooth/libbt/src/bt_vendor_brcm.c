...
const char *opcode_to_str(bt_vendor_opcode_t opcode)
{
    switch(opcode)
    {
	...
	case BT_VND_OP_SET_AUDIO_STATE:  return "BT_VND_OP_SET_AUDIO_STATE";
        ...
        case BT_VND_OP_A2DP_OFFLOAD_START:  return "BT_VND_OP_A2DP_OFFLOAD_START";
        case BT_VND_OP_A2DP_OFFLOAD_STOP:   return "BT_VND_OP_A2DP_OFFLOAD_STOP";
    }
    return "UNKNOWN";
}
...
static int op(bt_vendor_opcode_t opcode, void *param)
{
    ...
    BTVNDDBG("op for %s (%d)", opcode_to_str(opcode), opcode);
        ...
        case BT_VND_OP_POWER_CTRL:
            {
                ...
                {
                    BTVNDDBG("BT_VND_PWR_ON");
#if (USE_AXI_BRIDGE_LOCK == TRUE)
                    axi_bridge_lock(1);
#endif
                    upio_set_bluetooth_power(UPIO_BT_POWER_ON);
                } else {
                    BTVNDDBG("BT_VND_PWR_OFF");
        ...
        case BT_VND_OP_LPM_WAKE_SET_STATE:
            {
                ...
                uint8_t wake_assert = (*state == BT_VND_LPM_WAKE_ASSERT) ? \
                                        TRUE : FALSE;
                BTVNDDBG("wake_assert: %d", wake_assert);
