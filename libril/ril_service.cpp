...
int radio::getSignalStrengthResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        populateResponseInfo(responseInfo, serial, responseType, e);
        SignalStrength signalStrength = {};
        if (response == NULL || responseLen != sizeof(RIL_SignalStrength_BB)) {
...
int radio::getDataRegistrationStateResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        if (response == NULL) {
            ...
            int numStrings = responseLen / sizeof(char *);
            if ((numStrings != 6) && (numStrings != 11) && (numStrings != 12) && (numStrings != 13)) {
                RLOGE("getDataRegistrationStateResponse Invalid response: NULL");
                if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
            } else {
                char **resp = (char **) response;
...
void convertRilSignalStrengthToHal(void *response, size_t responseLen,
        SignalStrength& signalStrength) {
    RIL_SignalStrength_BB *rilSignalStrength = (RIL_SignalStrength_BB *) response;
    ...
    // RIL_SignalStrength_BB not support gw.timingAdvance. Set to INT_MAX as
    // invalid value.
